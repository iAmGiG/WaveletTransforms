    ...
    for layer in model.layers:
        if layer.trainable and layer.get_weights():
            ...
            for weight in weights:
                ...
                prune_indices = np.random.choice(flat_weights.size, prune_count, replace=False)
...
def main(original_model_path, prune_count, guid):
    model = load_model(original_model_path, original_model_path.replace('.h5', '_config.json'))
    csv_writer, csv_file = setup_csv_writer('random_pruning_log.csv')

    pruned_model, total_pruned = random_pruning(model, prune_count)
    log_pruning_details(csv_writer, guid, 'N/A', 'N/A', 'N/A', 'random', 'N/A', 'N/A', total_pruned, 'N/A')
